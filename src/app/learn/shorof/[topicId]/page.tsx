...
import { getShorofLesson } from '@/data/shorof/lessons';
...
export default function ShorofTopicPage() {
  const params = useParams();
  const topicId = params.topicId as string;
  const { language, t } = useLanguage();

  const lesson = getShorofLesson(topicId);
            ...
            <Card key={index} variant="default">
              <CardContent className="pt-6">
                ...
                {section.type === 'practice' && (
                  <div className="mt-4">
                    <Link href="/play/word-builder">
                      <Button className="w-full">
                        <Gamepad2 className="w-4 h-4 mr-2" />
                        {language === 'id' && 'Main Word Builder'}
                        {language === 'ar' && 'العب بناء الكلمات'}
                        {language === 'en' && 'Play Word Builder'}
                      </Button>
                    </Link>
                  </div>
                )}
